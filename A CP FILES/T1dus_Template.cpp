...
void solvethetestcase() {
    ...
    FORD(i, n, 0) {
        REP(j, n / 2 + 1) {
            REP(k, a[i] + 1) {
                if (j + a[i] - 2 * k > n / 2) continue;
                if (j + a[i] - 2 * k < 0) break;
                if (j - k < 0) break;
                if (k > 1) break;
                if (rem == j + a[i] - 2 * k) {
                    dp[i][j] += dp[i + 1][j + a[i] - 2 * k] * ncr(j + a[i] - 2 * k, a[i] - k);
                }
                else {
                    dp[i][j] += dp[i + 1][j + a[i] - 2 * k] * ncr(j + a[i] - 2 * k - 1, a[i] - k);
                }
                dp[i][j] %= FMOD;
            }
        }
        if (i <= mx and i > mn) dp[i][0] = 0;
        rem -= a[i];
    }
    cout << dp[0][0] << "\n";
}